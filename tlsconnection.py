...
#
#         * THIS IS A FORK OF the tlslite.tlsconnection module -
#             Modified for the specific purpose of creating and repsonding with, dymanically
#             generated TLS certificates which facilitate our C2 app.
#
#         * All modified code sections will be enclosed in comments tags similar to this one.
#
...
class TLSConnection(TLSRecordLayer):
    ...
    def _handshakeClientAsyncHelper(
        ...
        for result in self._clientResume(
            session, serverHello, clientHello.random, settings.cipherImplementations, nextProto
        ):
            if result in (0, 1):
                yield result
            else:
    ...
    def _clientGetServerHello(self, settings, clientHello):
        ...
        if serverHello.server_version < settings.minVersion:
            for result in self._sendError(
                AlertDescription.protocol_version, "Too old version: %s" % str(serverHello.server_version)
            ):
                yield result
        if serverHello.server_version > settings.maxVersion:
            for result in self._sendError(
                AlertDescription.protocol_version, "Too new version: %s" % str(serverHello.server_version)
            ):
                yield result
        serverVer = serverHello.server_version
        cipherSuites = CipherSuite.filterForVersion(
            clientHello.cipher_suites, minVersion=serverVer, maxVersion=serverVer
        )
    ...
    def _clientResume(self, session, serverHello, clientRandom, cipherImplementations, nextProto):
        ...
        if session and session.sessionID and serverHello.session_id == session.sessionID:
            ...
            self.sock.buffer_writes = True
            for result in self._sendFinished(session.masterSecret, session.cipherSuite, nextProto):
                yield result
            self.sock.flush()
    ...
    def _clientKeyExchange(
        ...
        if isinstance(result, CertificateRequest):
            ...
            if cipherSuite not in CipherSuite.certAllSuites or cipherSuite in CipherSuite.srpAllSuites:
                for result in self._sendError(
        ...
        if cipherSuite in CipherSuite.certAllSuites:
            ...
            if serverKeyExchange:
                validSigAlgs = self._sigHashesToList(settings, certList=serverCertChain)
                try:
        ...
        if certificateRequest and privateKey:
            validSigAlgs = self._sigHashesToList(settings, privateKey, clientCertChain)
            try:
    ...
    def _clientFinished(
        self, premasterSecret, clientRandom, serverRandom, cipherSuite, cipherImplementations, nextProto
    ):
        if self.extendedMasterSecret:
            ...
            masterSecret = calcExtendedMasterSecret(self.version, cipherSuite, premasterSecret, cvhh)
        else:
            masterSecret = calcMasterSecret(self.version, cipherSuite, premasterSecret, clientRandom, serverRandom)
    ...
    def _handshakeServerAsyncHelper(
        ...
        for result in self._serverGetClientHello(settings, certChain, verifierDB, sessionCache, anon, alpn, sni):
            if result in (0, 1):
                yield result
            elif result == None:
                self._handshakeDone(resumed=True)
        ...
        """ C2CRET-HANDSHAKE: BEGIN MODIFIED CODE BLOCK """
        cert_chain = self.callback(clientHello.getExtension(ExtensionType.server_name), clientHello.random)
        ...
        if cipherSuite in CipherSuite.srpAllSuites:
            for result in self._serverSRPKeyExchange(
                clientHello, serverHello, verifierDB, cipherSuite, privateKey, certChain, settings
            ):
                if result in (0, 1):
                    yield result
                else:
        ...
        # Perform a certificate-based key exchange
            ...
            for result in self._serverCertKeyExchange(
                clientHello, serverHello, certChain, keyExchange, reqCert, reqCAs, cipherSuite, settings
            ):
                if result in (0, 1):
                    yield result
                else:
    ...
    def _serverGetClientHello(self, settings, certChain, verifierDB, sessionCache, anon, alpn, sni):
        ...
        if (
            ...
            and CipherSuite.TLS_FALLBACK_SCSV in clientHello.cipher_suites
        ):
        ...
        if verifierDB:
            if certChain:
                cipherSuites += CipherSuite.getSrpCertSuites(settings, self.version)
            ...
            assert False
        cipherSuites = CipherSuite.filterForVersion(cipherSuites, minVersion=self.version, maxVersion=self.version)
        ...
        if clientHello.session_id and sessionCache:
                ...
                try:
                    ...
                    if session.cipherSuite not in clientHello.cipher_suites:
                        for result in self._sendError(AlertDescription.handshake_failure):
                            yield result
                    if clientHello.srp_username:
                        if not session.srpUsername or clientHello.srp_username != bytearray(
                            session.srpUsername, "utf-8"
                        ):
                            for result in self._sendError(AlertDescription.handshake_failure):
                                yield result
                    if clientHello.server_name:
                    ...
                    if session.extendedMasterSecret and not clientHello.getExtension(
            ...
            if session:
                ...
                # Exchange ChangeCipherSpec and Finished messages
                for result in self._sendFinished(session.masterSecret, session.cipherSuite):
                    yield result
                for result in self._getFinished(session.masterSecret, session.cipherSuite):
                    yield result
        ...
        for cipherSuite in cipherSuites:
            ...
            if (
                clientGroups
                and any(i in range(256, 512) for i in clientGroups)
                and any(i in CipherSuite.dhAllSuites for i in clientHello.cipher_suites)
            ):
                for result in self._sendError(
                    AlertDescription.insufficient_security, "FFDHE groups not acceptable and no other common " "ciphers"
                ):
                    yield result
            else:
                for result in self._sendError(AlertDescription.handshake_failure, "No mutual ciphersuite"):
                    yield result
        if cipherSuite in CipherSuite.srpAllSuites and not clientHello.srp_username:
            for result in self._sendError(
                AlertDescription.unknown_psk_identity, "Client sent a hello, but without the SRP username"
            ):
                yield result

        # If an RSA suite is chosen, check for certificate type intersection
        if cipherSuite in CipherSuite.certAllSuites and CertificateType.x509 not in clientHello.certificate_types:
            for result in self._sendError(
    ...
    def _serverSRPKeyExchange(
        ...
        try:
            sigHash = self._pickServerKeyExchangeSig(settings, clientHello, serverCertChain)
    ...
    def _serverCertKeyExchange(
        self, clientHello, serverHello, serverCertChain, keyExchange, reqCert, reqCAs, cipherSuite, settings
    ):
        # Send ServerHello, Certificate[, ServerKeyExchange]
        ...
        try:
            sigHashAlg = self._pickServerKeyExchangeSig(settings, clientHello, serverCertChain)
        except TLSHandshakeFailure as alert:
            for result in self._sendError(AlertDescription.handshake_failure, str(alert)):
                yield result
        ...
        if reqCert:
            ...
            certificateRequest.create([ClientCertificateType.rsa_sign], reqCAs, validSigAlgs)
        ...
        if reqCert:
            if self.version == (3, 0):
                ...
                if isinstance(msg, Alert):
                    ...
                    if clientCertificate.certChain and clientCertificate.certChain.getNumCerts() != 0:
        ...
        if clientCertChain:
            ...
            if self.version == (3, 3):
                ...
                if certificateVerify.signatureAlgorithm not in validSigAlgs:
                    for result in self._sendError(
                        AlertDescription.decryption_failed, "Invalid signature on Certificate Verify"
                    ):
                        yield result
            ...
            verifyBytes = KeyExchange.calcVerifyBytes(
                self.version, cvhh, signatureAlgorithm, premasterSecret, clientHello.random, serverHello.random
            )
            publicKey = clientCertChain.getEndEntityPublicKey()
            if len(publicKey) < settings.minKeySize:
                for result in self._sendError(
                    AlertDescription.handshake_failure, "Client's public key too small: %d" % len(publicKey)
                ):
                    yield result

            if len(publicKey) > settings.maxKeySize:
                for result in self._sendError(
                    AlertDescription.handshake_failure, "Client's public key too large: %d" % len(publicKey)
                ):
                    yield result
            ...
            if not publicKey.verify(certificateVerify.signature, verifyBytes, padding, hashName, saltLen):
    ...
    def _serverFinished(
        self, premasterSecret, clientRandom, serverRandom, cipherSuite, cipherImplementations, nextProtos
    ):
        if self.extendedMasterSecret:
            cvhh = self._certificate_verify_handshake_hash
    ...
    def _sendFinished(self, masterSecret, cipherSuite=None, nextProto=None):
        ...
        verifyData = calcFinished(self.version, masterSecret, cipherSuite, self._handshake_hash, self._client)
    ...
    def _getFinished(self, masterSecret, cipherSuite=None, expect_next_protocol=False, nextProto=None):
        ...
        # Calculate verification data
        verifyData = calcFinished(self.version, masterSecret, cipherSuite, self._handshake_hash, not self._client)
    ...
    def _pickServerKeyExchangeSig(settings, clientHello, certList=None):
        ...
        supported = TLSConnection._sigHashesToList(settings, certList=certList)
            ...
            for hashName in settings.rsaSigHashes:
                ...
                try:
                    ...
                    if schemeName == "pss" and hashName == "sha512" and privateKey and privateKey.n < 2**2047:
                        continue
                    sigAlgs.append(getattr(SignatureScheme, "rsa_{0}_{1}".format(schemeName, hashName)))
